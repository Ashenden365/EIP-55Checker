...
INVALID_SAMPLE = flip_case_on_first_alpha(VALID_SAMPLE)
...
def get_status_heading_and_text(result: dict) -> tuple[str, str]:
    ...
    if status == "checksum_ok":
        return (
            "✅ Checksum OK",
            "入力アドレスは、EIP-55 チェックサム表記として正しいです。",
        )

    if status == "checksum_mismatch":
        return (
            "❌ Checksum NG",
            "入力文字列は 20 バイトの16進アドレスとしては有効ですが、EIP-55 の大文字・小文字ルールには一致していません。",
        )
...
def render_status_message(result: dict) -> None:
    ...
    if status == "checksum_ok":
        st.success("Checksum OK: 入力アドレスは EIP-55 チェックサム表記として正しいです。")
    elif status == "checksum_mismatch":
        st.error(
            "Checksum NG: 入力文字列は 20 バイトの16進アドレスとしては有効ですが、"
            "EIP-55 の大文字・小文字ルールには一致していません。"
        )
...
def render_top_summary(result: dict) -> None:
    ...
    col1.metric("形式", "OK")
    col2.metric("EIP-55", "OK" if result["is_checksum_match"] else "NG")
...
def render_diff_section(result: dict) -> None:
    ...
    st.markdown(
        ...
        「入力された表記」と「正しい EIP-55 表記」の差分を確認できます。
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        ...
        <div class="legend-box">
            <span class="legend-item"><span class="legend-swatch legend-upper"></span>緑: 正しい表記では大文字になる英字</span>
            <span class="legend-item"><span class="legend-swatch legend-mismatch"></span>赤: 入力値で修正が必要な文字</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def render_details_section(result: dict) -> None:
    with st.expander("詳細を見る", expanded=False):
        ...
        st.caption(
            "EIP-55 では、先頭の 0x を外し、小文字化した 40 文字の16進文字列を "
            "Keccak-256 にかけ、その結果に応じて英字の大文字・小文字を決めます。"
        )
...
st.markdown(
    '<div class="intro-text">EVM 系ウォレットアドレスが EIP-55 チェックサム表記として正しいかどうかを判定します。</div>',
    unsafe_allow_html=True,
)
...
st.markdown(
    ...
    ・このアプリが確認するのは「アドレス値そのもの」ではなく、EIP-55 に沿った表記かどうかです。<br>
    ・したがって、EIP-55 として不正でも、アドレス値自体が存在しないとは限りません。
    </div>
    """,
    unsafe_allow_html=True,
)
