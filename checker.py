...
def _build_char_analysis(input_body: str, normalized_body: str, hash_hex: str) -> tuple[list[dict[str, Any]], str]:
...
def analyze_address(address: str) -> dict[str, Any]:
    ...
    try:
        hash_hex = keccak(text=normalized_body).hex()
    except ImportError as exc:
        base_result["error_message"] = (
            "Keccak-256 の計算に必要なハッシュバックエンドが未導入です。"
            ' requirements.txt に `eth-hash[pycryptodome]` を追加し、'
